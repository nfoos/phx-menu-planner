defmodule MenuPlannerWeb.Api.V1.AuthControllerTest do
  ...
  test "valid email and pass", %{conn: conn, user: %User{email: email}, swagger_schema: schema} do
    conn =
      conn
      |> post(Routes.api_v1_auth_path(conn, :login), email: email, password: @pass)
      |> validate_resp_schema(schema, "LoginResponse")
